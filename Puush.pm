...
my %upload_plugin_info = (
    'module'						=> "Puush",					#edit (must be the same as 'package')
	'url'						=> "http://puush.me/",				#edit (the website's url)
	'registration'  				=> "http://puush.me/register",			#edit (a link to the registration page)
	'name'						=> "Puush",					#edit (the provider's name)
	'description'					=> "Upload screenshots to Puush",		#edit (a description of the service)
...
#	}
	...
	eval{
		...
		#0,http://puu.sh/iMdDb/1f57b3de2c.png,277466193,0
		if($rsp == '-1'){
			$self->{_links}{'status'} = 'Upload Failed';
		}elsif($rsp == '-2'){
			$self->{_links}{'status'} = 'No Filename Header';
		}else{
			$self->{_links}->{'direct_link'} = (split(',', $rsp->content))[1];
